...
**`community_posts`** — `id`, `product_id`, `user_id`, `parent_post_id` (nullable, for replies), `type` (`question` | `answer` | `experience` | `update`), `body`, `trait_tags text[]`, `is_verified_owner`, `helpful_count`, `status` (`visible` | `flagged` | `removed`).
...
- `products`, `listings`, `literacy_entries`, `video_cache` → public read, **write restricted to service role** (Edge Functions only)
- `community_posts` → public read where `status = 'visible'`; insert requires authenticated user; update/delete only own post
...
3. **Confidence Score never ships without its explanation.** No bare number anywhere in the UI.
...
6. **Notifications must reference real user-specific data.** No generic "come back!" pushes. If the underlying data (e.g. `typical_duration_days`) is null, send nothing rather than guess.
7. **Streaks attach to routine completion only**, never to app opens, and copy never uses loss/guilt framing.
8. **No variable-reward, infinite-scroll, fake-scarcity, or countdown mechanics.** Anywhere.
9. **Never render numbers we can't substantiate.** "127 people tracking this" must be a real count query. No inflated or placeholder social proof.
10. **Secrets never reach the client.** Shopify tokens and the service role key live in Edge Functions only.
