...
-- 2. categories - Controlled category hierarchy with template assignments (no FK initially)
...
-- 3. intelligence_templates - Universal and category-specific templates (no FK initially)
...
create table if not exists public.intelligence_fields (
  ...
  refresh_policy_id text,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  updated_at timestamp with time zone default timezone('utc'::text, now()) not null
);

-- 5. intelligence_claims - Actual structured facts known about a product
...
-- 6. evidence - Source material supporting claims
...
-- 7. claim_evidence - Connects claims to one or more evidence items
...
-- 8. product_images - Primary and gallery images plus provenance
...
-- 9. product_prices - Observed prices with source and timestamp
...
-- 10. intelligence_jobs - Tracks research/refresh work
...
create index if not exists idx_intelligence_templates_category_id on public.intelligence_templates(category_id);
create index if not exists idx_intelligence_templates_parent_template_id on public.intelligence_templates(parent_template_id);
...
create trigger touch_intelligence_templates_updated_at before update on public.intelligence_templates for each row execute function public.touch_updated_at();

drop trigger if exists touch_intelligence_fields_updated_at on public.intelligence_fields;
create trigger touch_intelligence_fields_updated_at before update on public.intelligence_fields for each row execute function public.touch_updated_at();

drop trigger if exists touch_intelligence_claims_updated_at on public.intelligence_claims;
create trigger touch_intelligence_claims_updated_at before update on public.intelligence_claims for each row execute function public.touch_updated_at();

drop trigger if exists touch_product_images_updated_at on public.product_images;
create trigger touch_product_images_updated_at before update on public.product_images for each row execute function public.touch_updated_at();

drop trigger if exists touch_product_prices_updated_at on public.product_prices;
create trigger touch_product_prices_updated_at before update on public.product_prices for each row execute function public.touch_updated_at();

drop trigger if exists touch_intelligence_jobs_updated_at on public.intelligence_jobs;
create trigger touch_intelligence_jobs_updated_at before update on public.intelligence_jobs for each row execute function public.touch_updated_at();

drop trigger if exists touch_category_proposals_updated_at on public.category_proposals;
create trigger touch_category_proposals_updated_at before update on public.category_proposals for each row execute function public.touch_updated_at();
...
-- RLS Policies
create policy "Categories are readable by all" on public.categories for select using (status = 'active');
create policy "Categories are writable by service role" on public.categories for all using (auth.role() = 'service_role');
create policy "Templates are readable by all" on public.intelligence_templates for select using (status = 'active');
create policy "Templates are writable by service role" on public.intelligence_templates for all using (auth.role() = 'service_role');
create policy "Fields are readable by all" on public.intelligence_fields for select using (true);
create policy "Fields are writable by service role" on public.intelligence_fields for all using (auth.role() = 'service_role');
create policy "Claims are readable by all" on public.intelligence_claims for select using (status in ('supported', 'verified'));
create policy "Claims are writable by service role" on public.intelligence_claims for all using (auth.role() = 'service_role');
create policy "Evidence is readable by all" on public.evidence for select using (true);
create policy "Evidence is writable by service role" on public.evidence for all using (auth.role() = 'service_role');
create policy "Claim evidence is readable by all" on public.claim_evidence for select using (true);
create policy "Claim evidence is writable by service role" on public.claim_evidence for all using (auth.role() = 'service_role');
create policy "Product images are readable by all" on public.product_images for select using (true);
create policy "Product images are writable by service role" on public.product_images for all using (auth.role() = 'service_role');
create policy "Product prices are readable by all" on public.product_prices for select using (true);
create policy "Product prices are writable by service role" on public.product_prices for all using (auth.role() = 'service_role');
create policy "Jobs are readable by all" on public.intelligence_jobs for select using (true);
create policy "Jobs are writable by service role" on public.intelligence_jobs for all using (auth.role() = 'service_role');
create policy "Refresh policies are readable by all" on public.refresh_policies for select using (true);
create policy "Refresh policies are writable by service role" on public.refresh_policies for all using (auth.role() = 'service_role');
create policy "Category proposals are readable by all" on public.category_proposals for select using (true);
create policy "Category proposals are writable by service role" on public.category_proposals for all using (auth.role() = 'service_role');
